...
public class UserInterface {
    ...
    private void init(){
        ...
        if (this.dealership == null) {
            ...
            System.out.println("Please ensure 'dealership.json' exists and has the correct format.");
    ...
    public void display(){
        ...
        while(isRunning){
            ...
            System.out.println("10. Create a sales contract");
            System.out.println("11. Create a lease contract");
    ...
    public void processSalesContractRequest() {
        Vehicle vehicle = promptForVehicleByVin();
        if (vehicle == null) {
            return;
        }
        ...
        try {
            ...
            System.out.println("Will the customer finance? (y/n):");
            boolean financed = scanner.nextLine().trim().equalsIgnoreCase("y");

            String date = LocalDate.now().toString();
            SalesContract contract = new SalesContract(date, customerName, customerEmail, vehicle, financed);

            displaySalesContractSummary(contract);
            completeContract(contract);
        } catch (Exception ex) {
            System.out.println("Could not create sales contract: " + ex.getMessage());
        }
    }
    ...
    public void processLeaseContractRequest() {
        ...
        try {
            ...
            displayLeaseContractSummary(contract);
            completeContract(contract);
        } catch (Exception ex) {
            System.out.println("Could not create lease contract: " + ex.getMessage());
        }
    }

    private Vehicle promptForVehicleByVin() {
        try {
            Scanner scanner = new Scanner(System.in);
            System.out.println("Enter the VIN of the vehicle for this contract:");
            int vin = scanner.nextInt();
            scanner.nextLine();
    ...
    private void displaySalesContractSummary(SalesContract contract) {
        System.out.println("--- Sales Contract ---");
        System.out.printf("Customer: %s (%s)%n", contract.getCustomerName(), contract.getCustomerEmail());
        System.out.printf("Vehicle: %s%n", contract.getVehicleSold());
        printMoney("Sales tax:", contract.getSalesTaxAmount());
        printMoney("Recording fee:", contract.getRecordingFee());
        printMoney("Processing fee:", contract.getProcessingFee());
        printMoney("Total:", contract.getTotalPrice());
        if (contract.getMonthlyPayment().compareTo(BigDecimal.ZERO) == 0) {
            System.out.println("Monthly payment: Paid in full");
        } else {
            printMoney("Monthly payment:", contract.getMonthlyPayment());
        }
    }

    private void displayLeaseContractSummary(LeaseContract contract) {
        System.out.println("--- Lease Contract ---");
        System.out.printf("Customer: %s (%s)%n", contract.getCustomerName(), contract.getCustomerEmail());
        System.out.printf("Vehicle: %s%n", contract.getVehicleSold());
        printMoney("Expected ending value:", contract.getExpectedEndingValue());
        printMoney("Lease fee:", contract.getLeaseFee());
        printMoney("Total:", contract.getTotalPrice());
        printMoney("Monthly payment:", contract.getMonthlyPayment());
    }
    ...
    private void completeContract(Contract contract) {
        ...
        contractFileManager.appendContract(contract);
    ...
    private void printMoney(String label, BigDecimal amount) {
